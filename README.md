...
- **信任列表**：用户当日连续通过 N 次 AI 检测后加入信任列表，当日免检，降低 API 成本
...
- **误判纠正**：AI 错误拦截时，管理员点击通知卡片上的"放行"按钮自动解除黑名单并记录经验
- **漏判纠正**：AI 漏放垃圾信息时，管理员在群组**回复该消息并发送 `/markspam`**，自动拉黑、熔断信任度并更新记忆
- **自动抽象**：利用 LLM 自动总结提炼规则，不撑爆 Prompt，举一反三效果好
- **零外部依赖**：基于 D1 数据库和 LLM 自建记忆层，无额外成本，随时开关
